/**
 * Noosphere Reflect - Gemini Native Copy Button Scraper
 *
 * Platform-specific approach: Optimized for Gemini.google.com
 * Intercepts native "Copy" buttons to collect messages with Noosphere metadata.
 * Special handling for thinking blocks and expandable content.
 *
 * Instructions:
 * 1. Open a chat on Gemini.google.com
...
(function() {
    ...
    /**
     * Extract thoughts from message if present (Gemini thinking blocks)
     */
    ...
    function getPageMetadata() {
        ...
        let titleEl = document.querySelector('.conversation-title');
        let title = titleEl ? titleEl.innerText : null;

        // Fallback to page title
        if (!title) {
            titleEl = document.querySelector('title');
            title = titleEl ? titleEl.innerText : 'Gemini Conversation';
        }

        return {
            title: title.replace(` - ${CONFIG.PLATFORM_NAME}`, '').trim() || 'Gemini Conversation',
    ...
    function exportAsJSON(messages = null) {
        ...
        const chatData = {
            ...
            exportedBy: {
                ...
                version: 'Gemini Native Scraper v2.0',
    ...
    function exportAsMarkdown(messages = null) {
        ...
        msgs.forEach((msg) => {
            ...
            // Handle thoughts if present (Gemini thinking blocks)
        ...
        markdown += `*Exported by Noosphere Reflect - Gemini Native Scraper v2.0*\n`;
    ...
    function interceptNativeCopyButtons() {
        document.addEventListener('click', function(e) {
            ...
            if (target && target.textContent.toLowerCase().includes('copy')) {
                ...
                let messageContainer = target.closest('.turn') ||
                                     target.closest('[class*="user-query"]') ||
                                     target.closest('[class*="model-response"]');

                if (messageContainer) {
                    const text = extractMessageText(messageContainer);

                    if (text) {
                        // Determine if user or AI by checking classes
                        const isUser = messageContainer.classList.toString().includes('input') ||
    ...
    function init() {
        ...
        showStatus('✨ Gemini Native Scraper ready!', 'success');
