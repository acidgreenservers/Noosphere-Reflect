 ...
 *
 * Instructions:
 * 1. Open a chat on chat.mistral.ai
...
(function() {
    ...
    function getPageMetadata() {
        ...
        const title = titleEl ? titleEl.innerText : 'LeChat Conversation';

        return {
            title: title.replace(` - ${CONFIG.PLATFORM_NAME}`, '').trim() || 'LeChat Conversation',
    ...
    function exportAsJSON(messages = null) {
        ...
        const chatData = {
            ...
            exportedBy: {
                ...
                version: 'LeChat Native Scraper v2.0',
    ...
    function exportAsMarkdown(messages = null) {
        ...
        markdown += `*Exported by Noosphere Reflect - LeChat Native Scraper v2.0*\n`;
    ...
    function interceptNativeCopyButtons() {
        document.addEventListener('click', function(e) {
            ...
            if (target && target.textContent.toLowerCase().includes('copy')) {
                ...
                let messageContainer = target.closest('.ms-auto') ||
                                     target.closest('[class*="flex-col"]');

                if (messageContainer) {
                    const text = extractMessageText(messageContainer);

                    if (text) {
                        // Determine if user or AI by checking ms-auto (margin-left auto = user right-aligned)
                        const isUser = messageContainer.classList.toString().includes('ms-auto');
    ...
    function init() {
        ...
        showStatus('✨ LeChat Native Scraper ready!', 'success');
