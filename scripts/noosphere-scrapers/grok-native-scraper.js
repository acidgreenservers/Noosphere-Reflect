/**
 * Noosphere Reflect - Grok Native Copy Button Scraper
 *
 * Platform-specific approach: Optimized for Grok (grok.com / x.com)
 * Intercepts native "Copy" buttons to collect messages with Noosphere metadata.
 * Handles Grok's response-focused DOM structure.
 *
 * Instructions:
 * 1. Open a chat on Grok.com or x.com
...
(function() {
    ...
    function extractAllMessages() {
        const messages = [];
        // Note: Grok uses same selector for both user and AI messages
        // We detect direction from adjacent message, or assume alternating pattern
        const allMessages = document.querySelectorAll(CONFIG.SELECTORS.aiMsg);

        let messageType = 'prompt'; // Start with user
        allMessages.forEach((el, index) => {
            const text = extractMessageText(el);
            if (text) {
                messages.push(createMessageWithMetadata(messageType, text));
                // Alternate between user and AI
                messageType = messageType === 'prompt' ? 'response' : 'prompt';
            }
        });
    ...
    function getPageMetadata() {
        ...
        const title = titleEl ? titleEl.innerText : 'Grok Conversation';

        return {
            title: title.replace(` - ${CONFIG.PLATFORM_NAME}`, '').trim() || 'Grok Conversation',
    ...
    function exportAsJSON(messages = null) {
        ...
        const chatData = {
            ...
            exportedBy: {
                ...
                version: 'Grok Native Scraper v2.0',
    ...
    function exportAsMarkdown(messages = null) {
        ...
        markdown += `*Exported by Noosphere Reflect - Grok Native Scraper v2.0*\n`;
    ...
    function interceptNativeCopyButtons() {
        document.addEventListener('click', function(e) {
            ...
            if (target && target.textContent.toLowerCase().includes('copy')) {
                ...
                let messageContainer = target.closest('.response-content-markdown') ||
                ...
                if (messageContainer) {
                    ...
                    if (text) {
                        // For Grok, we'll prompt or auto-detect based on context
                        // Default to response type since Grok UI is response-focused
    ...
    function init() {
        ...
        showStatus('✨ Grok Native Scraper ready!', 'success');
