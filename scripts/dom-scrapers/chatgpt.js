...
(function () {
  ...
  const CONFIG = {
    ...
    NAMING_FORMAT_ID: 'chatgpt-naming-format',
    ...
    static createDropdown() {
      ...
      dropdown.innerHTML = `
        ...
        <div style="margin-top:14px;">
          ...
          <select id="${CONFIG.NAMING_FORMAT_ID}" style="margin-left:8px;padding:2px 8px;${inputStyles}">
  ...
  class ExportService {
    ...
    async buildMarkdown(turns, title) {
      ...
      turns.forEach((turn) => {
        ...
        if (userCheckbox?.checked || modelCheckbox?.checked) {
          selectedTurns.push(turn);
          if (userCheckbox?.checked) userCount++;
          if (modelCheckbox?.checked) aiCount++;
        }
      });
...
> **🌐 Date:** ${dateStr}
>
> **🌐 Source:** [ChatGPT Chat](${sourceUrl})
>
> **🏷️ Tags:** ChatGPT, AI-Chat, Noosphere
>
...
`;
      ...
      for (let i = 0; i < selectedTurns.length; i++) {
        ...
        if (userHeading && userCheckbox?.checked) {
          ...
          markdown += userContent || `[Could not read your message for turn ${i + 1}.]`;
          markdown += `\n\n`;
        }
        ...
        if (modelHeading && modelCheckbox?.checked) {
          ...
          if (copyBtn) {
            ...
            markdown += clipboardText || `[Could not copy the response for turn ${i + 1}.]`;
            markdown += `\n\n`;
          } else {
    ...
    async execute(mode, customFilename, namingFormat = 'kebab-case') {
      ...
      const filename = this.generateFilename(customFilename, title, namingFormat);

      await this.export(markdown, mode, filename);
    }
  }
  ...
  class ExportController {
    ...
    async handleButtonClick() {
      ...
      try {
        ...
        await this.exportService.execute(mode, customFilename, namingFormat);
