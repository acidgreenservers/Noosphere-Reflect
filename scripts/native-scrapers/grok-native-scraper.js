...
(function () {
    ...
    const CONFIG = {
        ...
        PLATFORM_NAME: 'Grok',
        PLATFORM_ID: 'grok',
        PLATFORM_COLOR: '#ec4899', // Grok Pink (Noosphere Style)
        SELECTORS: {
    ...
    function exportAsJSON(messages = null) {
        ...
        const chatData = {
            ...
            exportedBy: {
                ...
                version: 'Grok Native Scraper v2.2',
    ...
    function generateFilename(conversationTitle, format = 'kebab-case', extension = 'md') {
        ...
        const baseName = conversationTitle || 'Grok_Chat';
...
`;
    ...
    function extractAllMessages() {
        const elements = document.querySelectorAll(`${CONFIG.SELECTORS.userMsg}`);
        const messages = [];

        // Grok specific: Alternate pattern logic if needed, or just collect all matches
        elements.forEach((el, index) => {
            const type = index % 2 === 0 ? 'prompt' : 'response';
            const text = extractMessageText(el);
            if (text) messages.push(createMessageWithMetadata(type, text));
        });
        return messages;
    }
        ...
        consoleEl.innerHTML = `
            ...
            <div class="ns-content ns-console-content" id="ns-pane-config" style="display: none;">
                <div class="ns-config-group">
                    ...
                    <input type="text" class="ns-input" id="ns-custom-name" placeholder="Grok_Export">
                </div>
                <div>
    ...
    function interceptNativeCopyButtons() {
        document.addEventListener('click', (e) => {
            ...
            if (label.includes('copy') || text.includes('copy')) {
                ...
                if (container) {
                    // Default to response for single button clicks on Grok
                    const content = extractMessageText(container);
                    if (content) {
                        collectedMessages.push(createMessageWithMetadata('response', content));
