  ...
  generateHtml(
    ...
    const chatMessagesHtml = chatData.messages
      .map((message, index) => {
        ...
        let contentHtml = convertMarkdownToHtml(contentToRender, enableThoughts)
        ...
        // Support msg.thought if present
        if (!isPrompt && message.thought) {
          const formattedThought = convertMarkdownToHtml(message.thought, false)
            .replace(/<pre class="p-2 bg-gray-900 rounded-md my-2 overflow-x-auto">/g, `<pre class="p-2 ${codeBg} rounded-md my-2 overflow-x-auto">`)
            .replace(/<code class="language-/g, `<code class="${codeText} language-`)
            .replace(/<code class="inline-code">/g, `<code class="${codeBg} ${codeText} px-1 py-0.5 rounded text-sm">`)
            .replace(/<blockquote class="border-l-4 border-gray-500 pl-4 italic my-2">/g, `<blockquote class="border-l-4 ${blockquoteBorder} pl-4 italic text-gray-300 my-2">`);

          const thoughtBlockHtml = `
            <details class="markdown-thought-block my-4" open>
              <summary class="markdown-thought-summary cursor-pointer p-2 rounded-md flex items-center justify-between text-lg font-semibold bg-gray-600 text-gray-200 hover:bg-gray-500 active:bg-gray-700">
                Thought process: <span class="text-xs ml-2 opacity-70">(Click to expand/collapse)</span>
              </summary>
              <div class="markdown-thought-content p-3 border rounded-b-md bg-gray-700 text-gray-100 border-gray-600">
                ${formattedThought}
              </div>
            </details>
          `;
