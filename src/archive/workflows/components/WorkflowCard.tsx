...
export default function WorkflowCard({ workflow, viewMode = 'grid', isSelectionMode = false, onEdit, onDelete, onExport, onPreview, isSelected, onToggleSelect }: Props) {
        ...
        <div
            ...
            <div className="flex justify-between items-start mb-3 gap-4">
                <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 text-xs text-gray-400 mt-1 flex-wrap">
                        <span className={`px-2 py-0.5 rounded border ${getCategoryColor(workflow.metadata.category || 'General')}`}>
                            {workflow.metadata.category || 'General'}
                        </span>
                    </div>
                </div>
            ...
            <div className="flex items-center justify-between mt-auto pt-3 border-t border-white/5">
                <div className="flex items-center gap-2">
                    {workflow.metadata.exportStatus === 'modified' && (
                        <span className="text-[10px] text-amber-400/80" title={`Modified since export (${workflow.metadata.exportCount || 1} exports)`}>📝 Modified</span>
                    )}
                    {workflow.metadata.exportStatus === 'exported' && (
                        <span className="text-[10px] text-cyan-400/80" title={`Exported (${workflow.metadata.exportCount || 1} exports)`}>📤 Exported</span>
                    )}
                    {workflow.metadata.wordCount > 0 && (
                        <span className="text-[10px] text-gray-500" title="Word count">
                            {workflow.metadata.wordCount} words
                        </span>
                    )}
                </div>
