...
export const SkillPreviewModal: React.FC<SkillPreviewModalProps> = ({ skill, onClose, onSave }) => {
    const [searchTerm, setSearchTerm] = useState('');
    const [isEditing, setIsEditing] = useState(false);
    const [editedContent, setEditedContent] = useState(skill.content);
    const [editedTitle, setEditedTitle] = useState(skill.metadata.title);
                ...
                <div className="flex justify-between items-center p-6 border-b border-gray-800 shrink-0 bg-gray-900">
                    <div className="flex flex-col gap-1 flex-1 pr-4">
                        ...
                        <div className="flex items-center gap-3 text-xs text-gray-500">
                            ...
                            <span>•</span>
                            <span className="uppercase">{skill.metadata.category || 'General'}</span>
                            <span>•</span>
                            <span>{isEditing ? editedContent.split(/\s+/).length : skill.metadata.wordCount} words</span>
                        </div>
                    </div>
                    {/* Copy button */}
                    <button
                        onClick={() => {
                            const text = isEditing ? editedContent : skill.content;
                            ...
                            {isEditing ? (
                                <div className="p-4 bg-blue-900/20 border border-blue-500/30 rounded-xl">
                                    ...
                                    <p className="text-sm text-blue-200">
                                        You are currently editing this skill. Changes will update the word count automatically.
                                    </p>
                                    <button
                                        onClick={() => {
                                    ...
                                    <div className="bg-gray-900/50 rounded-xl border border-gray-800 p-4">
                                        ...
                                        <div className="space-y-3 text-sm">
                                            ...
                                            <div className="flex justify-between">
                                                ...
                                                <span className="text-gray-300">{new Date(skill.updatedAt).toLocaleDateString()}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span className="text-gray-500">Category</span>
                                                <span className="text-gray-300">{skill.metadata.category || 'General'}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span className="text-gray-500">Characters</span>
                                                <span className="text-gray-300">{skill.metadata.characterCount}</span>
                                            </div>
                                        </div>
                                    </div>
                                </>
                            ...
                            <div className="flex-1 overflow-y-auto p-4 lg:p-8 custom-scrollbar">
                                <div className="max-w-3xl mx-auto">
                                    <div className="prose prose-invert max-w-none">
                                        <div
                                            dangerouslySetInnerHTML={{ __html: renderMarkdownToHtml(skill.content) }}
